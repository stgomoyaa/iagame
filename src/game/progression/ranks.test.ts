...
describe('escalera de rangos', () => {
  ...
  it('son 25 rangos: 8 tiers de 3 divisiones mas Radiante', () => {
    expect(RANK_COUNT).toBe(25)
    expect(RANK_MIN).toBe(0)
    expect(RANK_MAX).toBe(24)
  })

  it('el primer rango es Hierro 1 y el ultimo es Radiante sin division', () => {
    expect(rankLabel(RANK_MIN)).toBe('Hierro 1')
    expect(rankName(RANK_MAX)).toEqual({ tier: 'Radiante', division: 0, label: 'Radiante' })
  })
  ...
  it('las divisiones van 1..3 y encadenan de un tier al siguiente', () => {
    ...
    expect(rankLabel(23)).toBe('Inmortal 3')
  })
  ...
  it('la dificultad va de 0 en Hierro 1 a 1 en Radiante', () => {
  ...
  it('clampea indices fuera de rango en vez de inventar tiers', () => {
    ...
    expect(rankLabel(999)).toBe('Radiante')
